...
- **Inspector 現在能播出瀏覽器不相容編碼的素材，且音頻預覽不再是白框。**
  ...
  - 純音頻檔本身可播，但舊實作把原生 `<audio controls>` 貼在預覽盒底部，瀏覽器/WebView 的
    原生 audio 控制條是白色長條，在深色預覽上看起來像白框、易被忽略；現改為自訂主題化播放器
    （置中圓形播放/暫停鈕 + 隱藏原生 audio + 時間碼）。
  - （codec.py / routers/misc.py:63 / frontend/src/lib/Inspector.svelte / MainLive.svelte /
    api.js 的 `buildProxyOne`/`getProxyStatus`）
